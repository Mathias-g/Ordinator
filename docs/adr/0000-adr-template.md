...
scope:                        # parts of Ordinator this decision touches
  - compiler
  - daemon
  - control-plane
interface-impact: none         # none | new | breaking  (config file format, CLI surface, or daemon control protocol)
---

<!--
Based on MADR 4.0.0 (https://adr.github.io/madr/), adapted for Ordinator.
...
implementation plan's phases (for example "Phase 3") or the current state of
the codebase; both drift. Anchor to the SPEC section instead (for example
"SPEC: The compiler"). See docs/adr/README.md, "What an ADR records".
-->
...
- Driver 1 (e.g. the definition must live in text files that are diffable and reviewable)
- Driver 2 (e.g. the SQLite file must hold only data, not schema)
- Driver 3
...
Examples: a test passes, a lint rule fails on violation, CI blocks a merge
that violates the rule.

## Interface notes

Fill this in only when `interface-impact` is `new` or `breaking`. Describe the
public contract being added or changed: which part of the config file format,
the CLI surface, or the daemon control protocol, and what consumers must do to
adapt. For a breaking change, note the migration path.
